Shader "STYLY/Examples/Raymarching/01_Sphere"
{
	Properties
	{
		...
		_BoxScale ("[Raymarching] Object Scale", Float) = 1.0
		_BoxSize ("[Raymarching] Object Size", Vector) = (1, 1, 1, 0)
	...
	SubShader
	{
		...
		Pass
		{
				...
				out float Hit, out float Distance, out float3 Normal)
			{
                ...
                // raymarching
                float3 ray = normalize(p.x * cSide + p.y * cUp + 1.0 * cDir); 
